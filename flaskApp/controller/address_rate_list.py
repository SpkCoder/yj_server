...
#/python/address_rate_list req_data { "action":"findData", "limit":10, "page":1, "whereJson":{}, "tocken": "eyJhbGciOiJIUzIbmFtZSmei4" }  //查询数据


class model(object):
    def __init__(self,req):
        self.req = req 
        self.table_name = 'address_rate_list'
    ...
    def find_data(self):
        ...
        if 'client_ip' in params['whereJson']:
            ...
            sql = 'select create_time as time, address_rate from address_rate_list where client_ip = "'+params['whereJson']['client_ip']+'" and create_time < "'+str_lt+'" and create_time >= "'+str_gte+'" order by create_time asc'
                ...
                for i in range(int_num+1):
                    ...
                    for item in result['rows']:
                        ...
                        if this_time <= int_gte + time_space*i:
                            list_rate.append(item['address_rate'])
                    if len(list_rate)>0:
                        result['address_rate'].append(round(sum(list_rate)/len(list_rate),2))
                    else:
                        result['address_rate'].append(0)
            ...
            # params['fieldJson'] = {"client_ip": 1,"vlan_id": 1,"vlan_name": 1,"vrf_name": 1,"log_time": 1,"create_time": 1}
            # params['groupJson'] = {"client_ip": 1, "address_rate":{"$avg": "address_rate","$avg_point": 2}}
